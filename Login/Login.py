...
def LoginGui():
    ...
    def login():
        ...
        select_query = "SELECT * FROM EmployeeAccount WHERE (EmployeeaccountID = (?) OR Username = (?)) and Password = (?)"
        c.execute(select_query, (username, username, password,))
        user = c.fetchone()
        if user is not None:
            # messagebox.showinfo('Login successfully', 'Logged in successfully')
            mGui.MainInterface(user[0], user[3], root)
            conn.commit()
